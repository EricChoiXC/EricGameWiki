...
登录行为：登录成功后写入 token，并按权限码批量预判接口回填当前用户权限（仅用于按钮显隐预判）；跳转回跳地址（无则进入首页）

强制改密：登录响应 `passwordExpired = true` 时，进入首页并自动弹出修改密码弹窗

访问控制：除登录页外，未登录访问任意页面重定向至登录页并携带回跳地址；已登录访问登录页回到首页；顶部栏"登出"清除本地登录态后回到登录页
